...
class UserApi extends MY_Controller
{
    ...
    // My own function
     public function myoauth(){
        ...
        // init configuration
        $clientID = '90221360861-opms5bv7nvglgra9927ls2gce58gad0a.apps.googleusercontent.com';
        $clientSecret = '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';
        $redirectUri = 'http://localhost/meramerchant';
    ...
    //     $user_data = [
    //         'username'   => 'skunkfinal',
    //         'passwd'     => 'PepfianlLePew7',
    //         'email'      => 'skufinal@example.com',
...
// public function verify_token($usertoken) {

//     $user_provided_token = $usertoken;
//     $stored_token = $this->session->userdata('user_token');
...
// }
    ...
    //       $sessionDetails = $this->session->userdata('userdetails');

    //               $this->output
    //               ->set_content_type('application/json')
    //               ->set_status_header('200')
    //               ->set_output(json_encode($_SESSION));
    //   }
    ...
    //               ->set_content_type('application/json')
    //               ->set_status_header('200')
    //               ->set_output(json_encode($sessionDetails));
    //   }
      ...
      public function login()
      {
        //   generate_token();
          ...
          $password = $decoded_data['passwd'];
            ...
            $row = $this->mymodel->queries($username,$password);
  
          if(!is_null($row)){
              $verify_password_status = password_verify($password, $row->passwd);
  
              if($verify_password_status==1){
                $result['message']= "Welcome ".$row->username;
                $result['token']= $this->generate_token();
                $token_data = array(
                    'email' => $row->email,
                    'token' => $result['token'],
                );
                $inserted_id = $this->mymodel->storeToken($token_data);
